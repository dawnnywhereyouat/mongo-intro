...
if __name__ == "__main__":
    ...
    print(
        list(cursor)
    )  # since the cursor is a generator, this wont be able to consume the data
    ...
    print(
        cursor
    )  # this wont work since id in a document is a ObjectId datatype, not a string
    cursor = todos_collection.find_one({"_id": ObjectId("641ab78c44ed5a53ba044b47")})
    print(cursor)  # the right way

    print(
        "Total documents in the collection:",
        todos_collection.count_documents(filter={}),
    )
